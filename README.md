...
```bash
# ดาวน์โหลด ZIP จาก GitHub → แตกไฟล์ → เปิด Terminal ในโฟลเดอร์
...
#### 3. เลือก Format และ Convert (ถ้า format ไม่ใช่ standard CSV)

| Format | ใช้กับ |
|--------|-------|
| **CSV/TSV (delimited)** | ไฟล์มี delimiter เช่น comma, tab, pipe (Final Fantasy 7 Remake) |
| **TXT (one per line)** | ไฟล์ `.txt` หนึ่งบรรทัดหนึ่งข้อความ (Uncharted 4) |
| **Subtitles (untranslated only)** | ไฟล์ที่มีทั้งแปลแล้ว+ยังไม่แปล สกัดเฉพาะที่ยังไม่แปล (Uncharted subtitles) |

1. เลือกไฟล์ต้นฉบับ
2. เลือก Format จาก dropdown
3. กดปุ่ม **Convert** (สีฟ้า) → ได้ไฟล์ `_standard.csv`
4. ไฟล์ที่แปลงจะถูกเลือกให้อัตโนมัติ
...
2. ได้ไฟล์ `ชื่อไฟล์เดิม_restored.นามสกุลเดิม` เช่น `2.common_restored.txt`
...
```bash
# Download ZIP from GitHub → extract → open Terminal in the folder
pip install -r src/requirements.txt
cd src
python app.py
```
...
| Format | Use for |
|--------|---------|
| **CSV/TSV (delimited)** | Files with comma, tab, pipe delimiters (Final Fantasy 7 Remake) |
| **TXT (one per line)** | One text per line `.txt` files (Uncharted 4) |
| **Subtitles (untranslated only)** | Mixed files — extracts only untranslated lines, merges back |

1. **Convert** (if needed): Select file → choose Format → click **Convert** (blue)
2. **Translate**: Set API key + model → click **Analyze & Translate**
3. **Restore** (if converted): Click **Restore** (purple) → `originalname_restored.ext`
